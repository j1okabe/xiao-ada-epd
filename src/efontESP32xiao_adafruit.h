...
//#define EFONT_DEBUG
//  comment out 　src/Adafruit_GFX.h line232 protected:
//  to access local value

void printEfont(char *str) {
  int posX = _TFT_OBJECT.getCursorX();
  int posY = _TFT_OBJECT.getCursorY();
  uint8_t textsize = _TFT_OBJECT.textsize_x;
...
void printEfont(char *str, int x, int y, int textsize) {
  _TFT_OBJECT.setCursor(x, y);
  _TFT_OBJECT.textsize_x = textsize;
  _TFT_OBJECT.textsize_y = textsize;
  printEfont(str);
}
